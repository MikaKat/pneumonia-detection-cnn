...
function Invoke-Run($label, $log, $arguments) {
    ...
    # Out-Host am Ende ist NICHT kosmetisch. Eine PowerShell-Funktion gibt
    # ALLES zurueck, was in den Ausgabestrom geschrieben wird, nicht nur das,
    # was hinter `return` steht. Ohne Out-Host reicht Tee-Object jede Logzeile
    # in den Rueckgabewert durch, `$code` wird ein Array aus dem ganzen Log und
    # der Vergleich `-ne 0` ist dann immer wahr: ein erfolgreicher Lauf meldet
    # sich als Fehlschlag. Genau das ist in der Nacht vom 01.08. passiert.
